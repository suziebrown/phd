...
Monte Carlo methods are a class of algorithms which use random samples to approximate integrals --- although they are usually associated with Bayesian inference in particular. 
...
Another class of Monte Carlo methods, and the focus of this work, is sequential Monte Carlo (SMC). Since its introduction to the statistics literature by \citet{gordon1993}, SMC has found a huge range of applications and has become an indispensible tool to practitioners from many fields. 
...
Sections \ref{sec:SMC} and \ref{sec:coalescent_theory} explain the background material on SMC and coalescent theory respectively. With these in place we are able to explain in Section \ref{sec:SMC_coal} the contribution of this work in the context of the SMC literature.
...
We assume that the \emph{transition} and \emph{emission} kernels have densities which we denote by $f$ and $g$ respectively, but this is not necessary in general.
...
This is vital if one wishes to carry out filtering in an on-line fashion, as it prevents the memory requirements accumulating more than necessary.

In the case of smoothing, if there are sufficient statistics available, we can store just these statistics at each time, plus the two most recent generations of particles, and throw away all other information about the particles at previous time steps. Even in this parsimonious case, the storage requirements will of course increase linearly with $T$.
...
A precise formulation of this notion is given in [REF].
...
Genealogies of the Cannings model also converge to the Kingman coalescent, under some conditions and a suitable time-scaling, as $N\to\infty$ \citep[Section 2.2]{etheridge2011}.
...
However, we do not achieve the same performance in the case of estimating the  smoothing distributions $p(x_{0:t} | y_{0:t})$. In this case we require a sample of \emph{trajectories} over times $0:t$ as opposed to a sample of particles at time $t$.
...
Variance estimation for SMC is an active research topic \citep{chan2013, lee2018, olsson2019}, but the previously proposed estimators require the output from at least one run of the algorithm, whereas our analysis allows us to estimate the variance \emph{a priori}.
...
& Y_t \mid (X_t = x_t) \sim g(\cdot | x_t) \qquad t=0,\dots,T
\end{align*}
It is plausible to assume additionally the conditional independence statement $(\Theta \indep Y_t) \mid X_t$, although this may not be necessary.
...
The derivations of the expressions \eqref{eq:cNtilde}, \eqref{eq:DNtilde}, \eqref{eq:cNtilde2}, along with details of the application of results from \citet{koskela2018}, are relegated to the appendix. 
...
\end{enumerate}
Condition \ref{item:resampling_unbiased} ensures that the resulting SMC estimates are unbiased. Condition \ref{item:resampling_random} is needed to prevent weight degeneracy. Conditions \ref{item:resampling_fixedN} and \ref{item:resampling_equalw} can be violated (for example \citet{liu1998} suggest sampling with the square roots of the weights and then correcting) but this is not generally advantageous.
...
\citet{crisan1999} introduce another resampling scheme based on a branching process, which they show to be optimal in some sense. However, their algorithm is not widely used in practice because it is much more complicated to implement than alternatives like systematic resampling which perform almost as well.
