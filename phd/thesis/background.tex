...
& \thead{support of $\nu_t^{(i)}$ given\\ $ k/N \leq w_t^{(i)} < (k+1)/N$} 
        ...
        & \thead{PRNG\\ calls} \\
\hline
\texttt{multi} & $\{0,\dots,N\}$ & $N$ & $\times$ & $\times$ & $\times$ & $N$ \\
\texttt{star} & $\{0, N\}$ & $N$ & $\times$ & $\times$ 
        & $\times$ & $1$ \\
\texttt{strat} & $\{k-1, k, k+1, k+2\}$ & $2$ & $\times$ & $\checkmark$ 
        & $\checkmark$ & $N$ \\
\texttt{syst} & $\{k, k+1\}$ & $1$ & $\checkmark$ & $\checkmark$ 
        & $\checkmark$ & $1$ \\
\texttt{res-multi} & $\{k,\dots,N\}$ & $N$ & $\times$ & $\checkmark$ 
        & $\times$ & $\leq N$ \\
\texttt{res-star} & $\{k, N\}$ & $N$ & $\times$ & $\checkmark$ 
        & $\times$ & $1$ \\
\texttt{res-strat} & $\{k, k+1, k+2\}$ & $2$ & $\times$ & $\checkmark$ 
        & $\checkmark$ & $\leq N$ \\
\texttt{res-syst} & $\{k, k+1\}$ & $1$ & $\checkmark$ & $\checkmark$ 
        & $\checkmark$ & $1$ \\
\texttt{ssp} & $\{k, k+1\}$? & $1$? & $\checkmark$? & $\checkmark$? 
        & $\checkmark$? & ? \\
\texttt{branching} & $\{k, k+1\}$? & $1$? & $\checkmark$? & $\checkmark$? 
        & $\checkmark$? & ? \\
\hline\hline
\end{tabular}
\caption[Properties of resampling schemes]{Properties of resampling schemes. The abbreviated names for the resampling schemes are explained in Table~\ref{tab:resampling_abbrevs}.}
...
Each particle $X_{t}^{(i)}$ is deterministically assigned $\flnw$ offspring and the remaining $R := N- \sum_{i=1}^N \flnw$ offspring are assigned stochastically according to the residual weights
\begin{equation*}
r^{(i)} := ( Nw_t^{(i)} - \flnw) /R .
\end{equation*}
...
\subsection{Stochastic rounding \seb{$\sim$} }
