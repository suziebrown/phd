...
\draft{Explain somewhere the purpose of the various conditions, e.g. on $g$ and $q$.}
\seb{Should the finite time-scale lemmas be propositions? They are not steps towards a larger result, the way the corollaries are currently stated, and they could be of independent interest...?}
...
For any $k\in\mathbb{N}$ the function $a_t^{(1:N)} \to (\nu_t^{(i)})_k$ is $\{i\}$-increasing in the sense of \textcite{koskela2018}, so we obtain the bounds
\begin{equation*}
\E[ (V_1^{(i)})_k ]
...
Furthermore, using the moments of the Binomial distribution \parencite[see for example][p.?]{mosimann1962}
\begin{equation*}
\E[ (V_1^{(i)})_k ]
...
now with time reversed and the notation made to fit with this study of genealogies, we obtain\seb{add one more step of working below to make it less ``magic''?}
\begin{equation*}
