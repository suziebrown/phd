...
\texttt{branching} & minimal variance branching algorithm \\%& \\
\hline\hline
\end{tabular}
...
\texttt{star} & $\{0, N\}$ & $N$ & $N^2/4$ & $\times$ & $\times$ 
        & $\times$ & $1$ \\
\texttt{strat} & $\{k-1, k, k+1, k+2\}$ & $2$ & $7/8$? & $\times$ & $\checkmark$ 
        & $\checkmark$ & $N$ \\
...
\end{itemize}
\seb{It might be useful to have these properties in a ref-able environment like a table or a proposition...}
...
\seb{This section was dumped from elsewhere and most of its subsections need redrafting. Also, definitely reorder the subsubsections!}


\subsubsection{Marginal variance of offspring counts}
\draft{Mention negative association, teaser for later, to do with covariance between counts rather than marginal variance.}
...
Table~\ref{tab:resampling_properties} includes upper bounds on $\V[\nu_t^{(i)}]$ for various resampling schemes, independent of $w_t^{(i)}$. Those general bounds are derived from the results of this section, bounded above independently of the weights.
...
\end{enumerate}
\end{prop}
Parts \ref{item:resampling_var1} and \ref{item:resampling_var2} were proved in \textcite[Section 3]{douc2005}. and the second inequality in \ref{item:resampling_var4} follows from \ref{item:resampling_var2}, as shown below.
...
\draft{(See \textcite{hol2006} for inspiration.) Include a diagram showing the quantity inside the $D^\star$ supremum, plotted over $u$, for mn/strat/syst?}
...
\seb{Even \texttt{star} and \texttt{SSP} and \texttt{branching}? If it turns out to differ depending on resampling scheme then include it as a column in Table~\ref{tab:resampling_properties}. --- I think \texttt{branching} is $O(N^2)$, if I understand \textcite[Corollary 18]{crisan1999} correctly.}
...
\subsection{Stochastic rounding \seb{$\sim$} }\label{sec:SRs}
\draft{Define stochastic rounding. Resampling schemes contained by this class. General properties for this class (marginal distributions, negative association, minimum-variance). State explicitly that marginal SR count = $\flnw$ + Bernoulli($Nw_t^{(i)} - \flnw$).}
...
Some of the properties discussed earlier are common to every stochastic rounding scheme. By definition the support of an offspring count $\nu_t^{(i)}$, if the associated weight lies in the interval $k/N \leq w_t^{(i)} < (k+1)/N$, is $\{ k, k+1\}$. All stochastic roundings are also degenerate by definition when the weights are all equal, i.e.\ $w_t^{(1:N)} = (1,\dots, 1)/N$.
