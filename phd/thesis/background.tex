...
\section{Sequential Monte Carlo \seb{$\checkmark$} }
...
\label{sec:FKmodels}
\draft{Example of non-SSM that is FK?}
...
\vspace*{10pt}
\DontPrintSemicolon
\KwData{$N, T, \mathbb{M}_0, (M_t)_{t=1}^T, (G_t)_{t=0}^T$}
\lFor{$i \in \{1,\dots,N\}$}{ 
...
\For{$t \in \{0,\dots, T-1\}$}{
	Sample $a_t^{(1:N)} \sim $ \textsc{resample}$(\{1,\dots ,N\}, w_t^{(1:N)}$)\;
	\lFor{$i \in \{1,\dots,N\}$}{
		Sample $X_{t+1}^{(i)} \sim M_{t+1}(X_t^{(a_t^{(i)})}, \cdot)$
	}
	\lFor{$i \in \{1,\dots, N\}$}{	
		$w_{t+1}^{(i)} \gets \Big\{ {\sum_{j=1}^N G_{t+1}(X_t^{(a_t^{(j)})},X_{t+1}^{(j)}) }\Big\}^{-1} G_{t+1}(X_t^{(a_t^{(i)})},X_{t+1}^{(i)}) $
	}
}
\vspace*{10pt}
...
Similar approximations can be constructed for prediction and fixed-lag smoothing\seb{...be explicit or no?}.
We can also approximate the marginal likelihood using the \emph{unnormalised} weights\seb{should I introduce notation e.g. $\tilde{w}$ for the unnormalised weights, and give them a separate line in Algorithm~\ref{alg:SMC}, and include them in the possible outputs of the algorithm?}:
...
Arrangements such that one or both ends have no overhang are special cases of Case \subref{fig:strat_case1}, with the $\delta_L \in \{0,\delta\}$.
...
%\end{tikzpicture}
%}\\
\subfloat[The overhang is less than one; $ \delta_L \in {[0, \delta]} $. The parent under consideration is automatically assigned $K$ offspring, plus up to two more.]{
\begin{tikzpicture}
% w subinterval
\draw[thick] (0,0)--(1.9,0);
\draw[thick,dotted] (1.9,0)--(2.3,0);
...
\subfloat[The overhang is greater than one (this case can only occur when $K\geq1$); $\delta_L \in (\delta,1)$. The parent under consideration is automatically assigned $K-1$ offspring, plus up to two more.]{
\begin{tikzpicture}
% w subinterval
...
To define a probability distribution over these four values, we introduce the notation $p_j := \Prob[ \nu_t^{(i)} = \flnw +j \mid w_t^{(i)} ]$, for $j=-1,0,1,2$.\seb{Change notation to avoid confusion with $p_k^{(i)}$'s in Chapter~\ref{ch:appl}?}
...
We can use the values of $p_{-1},p_0,p_1,p_2$ in the two cases of Figure~\ref{fig:strat_cases}\seb{these calculations will need updating since Table~\ref{tab:strat_probs} is now more informative}, as summarised in Table~\ref{tab:strat_probs}, to bound the variance. First write
\begin{align*}
\V[ \nu_t^{(i)} \mid w_t^{(i)} ]
...
\seb{Don't include proofs for things that were already proved by someone else, unless this really adds value.}
...
\subsection{Particle MCMC \seb{$\sim$} }
\seb{Redraft this subsection.}\\
...
\seb{Sort out notation e.g.\ $x^\star$ versus $X^\star$}


\subsection{Particle Gibbs algorithm \seb{$\checkmark$} }
\draft{note that of course the algorithm is more general than this peculiar example.}\\
...
\seb{In Section~\ref{sec:ancsamp_genealogy}, we require a time index on $q$, so maybe it's worth keeping the time dependence in this section too, e.g.\ $K_t^\theta$?}
...
%Here we take the \textsc{resample} procedure for the non-immortal particles to be Categorical sampling of parental indices (known as \emph{multinomial resampling}; see Section~??). It is possible to use other resampling schemes, but 
%\begin{algorithm}[ht]
%\vspace*{10pt}
%\DontPrintSemicolon
...
%        Sample $a_t^{(1:N)}\setminus a_t^\star \sim \Cat(\{1,\dots,N\}, w_t^{(1:N)})$\;
%	\lFor{$i \in \{1,\dots,N\}$}{
%		Sample $X_{t+1}^{(i)} \sim K_{t+1}(X_t^{(a_t^{(i)})}, \cdot)$
%	}
%    	\lFor{$i \in \{1,\dots, N\}$}{	
...
%}
%\vspace*{10pt}
%\caption[Conditional sequential Monte Carlo with multinomial resampling]{Conditional sequential Monte Carlo with multinomial resampling. The ``immortal particle'' at each time has its new state and parental index set deterministically according to the values of $x_{0:T}^\star$ and $a_{0:T}^\star$ given as input.}
%\label{alg:condSMC}
%\end{algorithm}
\begin{algorithm}[ht]
\vspace*{10pt}
\DontPrintSemicolon
\KwData{$N, T, \mu, K_\theta, g_\theta, x_{0:T}^\star, a_{0:T}^\star$}
Set $X_0^{(a_0^\star)} \gets x_0^\star$\;
\lFor{$i \in \{1,\dots,N\} \setminus a_0^\star$}{
	Sample $X_0^{(i)} \sim \mu(\cdot)$
}
\lFor{$i \in \{1,\dots, N\}$}{
		$w_{0}^{(i)} \gets  \left\{ {\sum_{j=1}^N g_\theta(X_0^{(j)})}\right \}^{-1}{g_\theta(X_0^{(i)})} $
}                
\For{$t \in \{0,\dots, T-1\}$}{
	Set $a_t^{(a_{t+1}^\star)} \gets a_t^\star$, 	$X_{t+1}^{(a_{t+1}^\star)} \gets x_{t+1}^\star$\;
        Sample $a_t^{(1:N)}\setminus a_t^\star \sim \textsc{resample}(\{1,\dots,N\}, w_t^{(1:N)})$\;
	\lFor{$i \in \{1,\dots,N\}$}{
		Sample $X_{t+1}^{(i)} \sim K_\theta(X_t^{(a_t^{(i)})}, \cdot)$
	}
    	\lFor{$i \in \{1,\dots, N\}$}{	
		$w_{t+1}^{(i)} \gets \Big\{ {\sum_{j=1}^N g_\theta(X_t^{(a_t^{(j)})},X_{t+1}^{(j)}) }\Big\}^{-1} g_\theta(X_t^{(a_t^{(i)})},X_{t+1}^{(i)}) $
	}          
}
\vspace*{10pt}
\caption[Conditional sequential Monte Carlo for a parametrised state space model]{Conditional sequential Monte Carlo for a parametrised state space model. The ``immortal particle'' at each time has its new state and parental index set deterministically according to the values of $x_{0:T}^\star$ and $a_{0:T}^\star$ given as input.}
\label{alg:condSMC}
\end{algorithm}
...
%% Turns out trying to write the algorithm explicitly is a nightmare.
%The complete particle Gibbs algorithm for this example is summarised in Algorithm~\seb{??}. As was eluded to earlier, it consists of alternately sampling from the full conditional distribution of $\theta$ (using a Metropolis-Hastings update) and sampling a trajectory $x_{0:T}$ using conditional SMC.
%\seb{put the alg here}
%
%\begin{algorithm}[ht]
%\vspace*{10pt}
%\DontPrintSemicolon
%\KwData{$M, N, T, \tilde{p}, \mu, K_\theta, g_\theta$}
%Set $X_0^{(a_0^\star)} \gets x_0^\star$\;
%\For{$i \in \{1,\dots,M\}$}{
%	Sample $\tilde{\theta} \sim \tilde{p}( \cdot \mid \theta[i-1] )$
%	Sample $U \sim \Unif[0,1]$
%	\If{$U \leq \frac{ p( \tilde{\theta} ) p(X[i], y_{0:T} \mid \tilde{\theta} ) \tilde{p}( theta[i-1] \mid \tilde{\theta} ) }{ p(\theta[i-1]) p(X[i], y_{0:T} \mid \theta[i-1]) \tilde{p}( \tilde{\theta} \mid \theta[i-1] ) }$}{
%	    Set $\theta[i] \gets \tilde{\theta}$
%	}
%	\Else{ $\theta[i] \gets \theta[i-1]$ }
%    Set $( (X_{T,0:T}^{(i)})_{i}, (w_T^{(i)})_{i} ) \gets$ \texttt{ConditionalSMC}$(N, T, \mu, K_\theta, g_\theta, X[i-1], A[i-1])$
%    Sample $j \sim \Cat(\{ 1,\dots,N \}, w_T^{(1:N)}$ %$X_T \sim \Cat(\{ X_T^{(1)}, \dots, X_T^{(N)} \}, w_T^{(1:N)}$
%    Set $X[i] \gets X_{T,0:T}^{(j)} $
%}
%\vspace*{10pt}
%\caption[Particle Gibbs for a parametrised state space model]{Particle Gibbs for a parametrised state space model}
%\label{alg:particleGibbs}
%\end{algorithm}
%
%
%%%%
...
To see this, consider the illustration in Figure~\ref{fig:PG_ancdegen}.\seb{don't need two panes as in Lindsten, just depict the one iteration with highlighting described below.} 
Figure \ref{fig:PG_ancdegen} represents the smoothing trajectories generated by the conditional SMC update at iteration $t$. The dark line is the immortal trajectory given as input, that is the trajectory sampled by the conditional SMC update at iteration $t-1$. Backwards in time, the sampled trajectories quickly coalesce until at time \seb{blah: the MRCA time in the Figure} all of the trajectories have coalesced. The common trajectory up to time\seb{blah} must necessarily be part of the immortal trajectory.
A new trajectory (highlighted) is then sampled among the $N$ trajectories shown. Whichever trajectory we sample, it must overlap with the previously sampled trajectory at least from time\seb{zero to blah}.
...
In the basic algorithm with multinomial resampling, at each time step the non-immortal particles are resampled by multinomial resampling according to their weights, while the immortal offspring is deterministically assigned to the immortal parent. That is, at time $t$, for each $i \in \{1,\dots, N\}$,\seb{these LHSs need some conditioning! is the conditioning in the next display correct?}
\begin{equation*}
\Prob[a_t^{(j)} = i] \propto 
\begin{cases}
...
Ancestor sampling alters the resampling step by including a backwards sampling\seb{simulation?} step for the immortal particle. Instead of deterministically inheriting the ``correct'' parent, the immortal particle samples its parent among all $N$ possible parents.
This is justified in the same way as backwards sampling in general (Section~??), provided the ancestor sampling probabilities are chosen correctly, although we now apply the backwards sampling step to the immortal trajectory only.
...
\Prob[a_t^{(j)} = i \mid X_{0:t-1}^{(1:N)}, x_{0:T}^\star, a_{0:T}^\star ] 
\propto \begin{cases}
w_t^{(i)} & j \text{ non-immortal}\\
w_t^{(i)} \frac{
        p( (X_{t-1, 0:t-1}^{(i)}, a_{0:t-1}^{(1:N)}, x_{t:T}^\star) \mid \theta, y_{0:T} )}
        {p( X_{t-1, 0:t-1}^{(i)} \mid \theta, y_{0:t-1} ) } 
        & j \text{ immortal} .
\end{cases}
\end{equation}
The ratio of densities can be interpreted as the conditional probability that the whole trajectory is the concatenation of $(X_{t-1, 0:t-1}^{(i)}$ with $x_{t:T}^\star)$, given that its first $t-1$ states are $X_{t-1, 0:t-1}^{(i)}$.
To simplify the ratio, we can use \eqref{eq:PG_Xposterior} to write
\begin{equation*}
p( X_{t-1, 0:t-1}^{(i)} \mid \theta, y_{0:t-1} )
\propto \mu_\theta( X_{t-1,0}^{(i)} ) g_\theta( y_0 \mid X_{t-1,0}^{(i)} ) 
        \prod_{s=1}^{t-1} q_\theta( X_{t-1,s}^{(i)} \mid X_{t-1,s-1}^{(i)} ) 
        g_\theta( y_s \mid X_{t-1,s}^{(i)} )
\end{equation*}
...
\begin{align*}
p( (X_{t-1, 0:t-1}^{(i)}, x_{t:T}^\star) \mid \theta, y_{0:T} )
&\propto \mu_\theta( X_{t-1,0}^{(i)} ) g_\theta( y_0 \mid X_{t-1,0}^{(i)} ) 
        \left\{ \prod_{s=1}^{t-1} q_\theta( X_{t-1,s}^{(i)} \mid X_{t-1,s-1}^{(i)} )
        g_\theta( y_s \mid X_{t-1,s}^{(i)} ) \right\} \\
    &\qquad\times q_\theta( x_{t}^\star \mid X_{t-1,t-1}^{(i)} ) 
        g_\theta( y_t \mid x_t^\star )
        \left\{ \prod_{s=t+1}^T q_\theta(x_s^\star \mid x_{s-1}^\star)
        g_\theta(y_s \mid x_s^\star) \right\} .
\end{align*}
The ratio then becomes\seb{I think the first proportionality hides the likelihood ratio $p(y_{0:T}|\theta)/p(y_{0:t-1}|\theta)$ --- see \eqref{eq:PG_Xposterior} --- it might make things clearer if I keep such constants explicit for longer}
\begin{align*}
\frac{p( ( X_{t-1,0:t-1}^{(i)}, x_{t:T}^\star ) \mid \theta, y_{0:T} )}
        {p( X_{t-1, 0:t-1}^{(i)} \mid \theta, y_{0:t-1} ) }
&\propto q_\theta( x_t^\star \mid X_{t-1,t-1}^{(i)} ) g_\theta( y_t \mid x_t^\star ) 
        \prod_{s=t}^T q_\theta( x_s^\star \mid x_{s-1}^\star ) 
        g_\theta( y_s \mid x_s^\star ) \\
&\propto q_\theta( x_t^\star \mid X_{t-1,t-1}^{(i)} ) \\
&= q_\theta( x_t^\star \mid X_{t-1}^{(i)} )  .
\end{align*}
The probabilities in \eqref{eq:ancsamp_probs} become
\begin{equation}
\Prob[a_t^{(j)} = i \mid X_{0:t-1}^{(1:N)}, X_{t:T}^\star ] \propto
\begin{cases}
w_t^{(i)} & j \text{ non-immortal}\\
w_t^{(i)} q_\theta( x_t^\star \mid X_{t-1}^{(i)} ) & j \text{ immortal} .
\end{cases}
\end{equation}
Therefore, in order to do ancestor sampling, we need a stronger assumption on the Markov kernels than was required to simply run the conditional SMC algorithm: we now require that $K_\theta$ admits a density $q_\theta$ and that this density can be evaluated pointwise, whereas previously we only needed to draw samples from $K_\theta$\seb{or rather from $K_\theta(\cdot \mid x)$ for all $x$}.
This additional requirement rules out ancestor sampling in some applications.

Recall that the usual backward simulation procedure requires a full forward pass to calculate the future states before the backward simulation probabilities can be computed. Ancestor resampling, on the other hand, does not require a forward pass because it only computes backward simulation probabilities for the immortal trajectory, for which all the future states are known in advance. This means that ancestor sampling can be implemented on-line.
...
The contribution of ancestor sampling is to break up the immortal lineage (see Figure~\ref{fig:whyASworks}), so that while the non-immortal trajectories may still coalesce, they no longer preferentially coalesce onto the immortal trajectory. 
