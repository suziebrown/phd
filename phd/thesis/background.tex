...
\vspace*{10pt}
\DontPrintSemicolon
\KwData{$N, T, \mu, q_\theta, g_\theta, x_{0:T}^\star, a_{0:T}^\star$}
Set $X_0^{(a_0^\star)} \gets x_0^\star$\;
\lFor{$i \in \{1,\dots,N\} \setminus a_0^\star$}{
	Sample $X_0^{(i)} \sim \mu(\cdot)$
}
...
\For{$t \in \{0,\dots, T-1\}$}{
	Set $X_{t+1}^{(a_{t+1}^\star)} \gets x_{t+1}^\star$\;
	Sample $a_t^{(a_{t+1}^\star)} \sim \Cat\left( \{1,\dots,N\}, w_t^{(1:N)} q_\theta(x_{t+1}^\star \mid X_{t}^{(1:N)}) \right)$\;
        Sample $a_t^{(1:N)}\setminus a_t^\star \sim \textsc{resample}(\{1,\dots,N\}, w_t^{(1:N)})$\;
	\lFor{$i \in \{1,\dots,N\}$}{
		Sample $X_{t+1}^{(i)} \sim q_\theta(X_t^{(a_t^{(i)})}, \cdot)$
	}
    	\lFor{$i \in \{1,\dots, N\}$}{	
		$w_{t+1}^{(i)} \gets \Big\{ {\sum_{j=1}^N g_\theta(X_t^{(a_t^{(j)})},X_{t+1}^{(j)}) }\Big\}^{-1} g_\theta(X_t^{(a_t^{(i)})},X_{t+1}^{(i)}) $
	}          
...
\vspace*{10pt}
\caption[Conditional sequential Monte Carlo with ancestor sampling]{Conditional sequential Monte Carlo with ancestor sampling for a parametrised state space model. The parent of the ``immortal particle'' is updated at each iteration via an on-line backward simulation step.}
...
\end{cases}
\end{equation*}
\seb{Refer to this equation in Applications chapter when I "magic" up the ancestor sampling result.}
