...
Since the above holds for all $i$, we have
\begin{equation*}
\frac{1}{(N)_3} \sum_{i=1}^{N} \E [(\nu_t^{(i)})_3 \mid \mathcal{H}_t ]
\leq \frac{a^2-1}{N-2} \frac{1}{(N)_2} \E [(\nu_t^{(i)})_2 \mid \mathcal{H}_t ]
\end{equation*}
...
\draft{This one doesn't fit under the SRs umbrella as previously claimed --- oops! Proof for this one is in progress, but shouldn't be too difficult. Like SRs, there are only finitely many possible counts conditional on weights, so the same kind of proof will work (but with four cases instead of two).}
