...
SMC was developed for inference in intractable state space models (details in Section\seb{??}) and introduced to the statistics community by \textcite{gordon1993}.
The basic idea behind SMC is sequential importance sampling, whereby the posterior importance samples from one target distribution are used to generate proposal samples for the next. A full derivation of the SMC recursions is beyond the scope of this work, but the reader is referred to\seb{[citations]} for more background. Here it will suffice to include a motivation in the context of state space models (Section\seb{??}) followed by the formalism of Feynman-Kac models (Section\seb{??}).





\subsection{State space models \seb{$\checkmark$} }
\label{sec:SSMs}
...
\node at (-2,0) {$\dots$};
\node at (5.1,0) {$\dots$};
\draw[->] (-1.7,0)-- node[anchor=north] {\footnotesize{$K_t$}} (-0.5,0);
...
\end{tikzpicture}
\caption[Graphical depiction of a general state space model]{Graphical depiction of a general state space model. $(X_t)$ is a Markov process with transition kernels $(K_t)$ representing the underlying state of the system. $Y_t$ is a noisy observation of $X_t$ for each $t$.\seb{Include also the $t-1$ column in this graph for greater clarity.}}
\label{fig:SSM}
\end{figure}
...
In certain cases, these inference problems may be solved analytically (Section\seb{??}), but this is not typically the case. For intractable models we must resort to Monte Carlo methods. However, state space inference is problematic even with Monte Carlo. 
...
We can also approximate the marginal likelihood by using the \emph{unnormalised} weights\seb{should I introduce notation e.g. $\tilde{w}$ for the unnormalised weights, and give them a separate line in Algorithm~\ref{alg:SMC}, and include them in the possible outputs of the algorithm?}:
\begin{equation*}
L_t 
...
%%%
\seb{Include here the dependence graph without the filtration and separatrix, and then include this embellished version later on when I first make reference to the D-separation argument (namely in Section~\ref{sec:SMC_genealogies} or just before). Then the graph version here should be labelled forward-in-time; currently it's labelled backwards which is just confusing for the reader.}\\

\seb{Include arrows to and from the "..." extensions? Might make it visually clearer that this is just a section of a larger graph.}\\
