...
One other interesting quantity is the probability that a random sample of $k$ lineages has the same as the MRCA as the entire population (of size $N$). As $N\to\infty$, this probability converges to $(k-1)/(k+1)$ \parencite[e.g.][Theorem 1.7]{durrett2008}.
To see this, consider the two subtrees produced by cutting the tree just below the population MRCA. The sample of $k$ lineages coalesces before the population MRCA if and only if all $k$ sampled leaves lie in just one of these two subtrees.
A basic consequence of the exchangeability of the $n$-coalescent is that the proportion of leaves in, say, the left subtree is uniformly distributed on $[0,1]$.
Calling this proportion $X$, we have
\begin{equation*}
\Prob [k \text{ random lineages all in one subtree} \mid X=x]
= x^k + (1-x)^k
\end{equation*}
Integrating against the distribution of $X$, the probability of interest is
\begin{equation*}
\Prob[k \text{-sample MRCA } = \text{ population MRCA} ]
...
Consider now a subsample of size $k$ among $n$ lineages that follow the $n$-coalescent. What is the probability that these $k$ lineages have the same MRCA as all $n$ lineages?
This question is addressed in \textcite[Example 1]{saunders1984} and again in \textcite[Equation (3)]{spouge2014}, in both cases arising as a special case of more general results.
The probability is 
\begin{equation*}
\frac{k-1}{k+1} \frac{n+1}{n-1}
\end{equation*}
and as $n\to\infty$ this agrees with the population-level result above.
\seb{Include a direct proof for the $k=2$ case?}
...
Several ways to enforce this constraint on the joint distribution have been proposed, including systematic resampling, residual resampling with stratified or systematic residuals, the branching system of \textcite{crisan1997}, and the Srinivasan sampling process resampling introduced in \textcite{gerber2017}.
