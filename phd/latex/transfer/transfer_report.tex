...
\end{equation*}
has mass in $N$ locations, and the Monte Carlo error for estimating expectations under $p(x_t | y_{0:t})$ scales as $O(N^{-1/2})$ [REF].
...
The problem of ancestral degeneracy can be mitigated by these various measures, but not entirely removed. Therefore it is natural to ask how bad we can expect it to be. 
...
In general, analysis of the genealogies can quantify \emph{a priori} the severity of ancestral degeneracy and thus the sampling variance to be expected with a given algorithm and number of particles.
