...
The correct resampling probabilities are, for each $i$,\seb{I think we have to assume $p$s (i.e. $\mu$ and $K$) admit densities at this point, and drop the $d$'s}
\begin{equation}\label{eq:ancsamp_probs}
\Prob[a_t^{(j)} = i \mid X_{0:t-1}^{(1:N)}, X_{t:T}^\star ] \propto
\begin{cases}
w_t^{(i)} & j \text{ non-immortal}\\
w_t^{(i)} \frac{p( d(x_{0:t-1}^{(i)}, x_{t:T}^*) \mid \theta, y_{0:T} )}
        {p( dx_{0:t-1}^{(i)} \mid \theta, y_{0:t-1} ) } 
        & j \text{ immortal} .
\end{cases}
\end{equation}
The ratio of densities can be interpreted as the conditional probability that the whole trajectory is $(x_{0:t-1}^{(i)}, x_{t:T}^*)$ given that the first $t-1$ states are $x_{0:t-1}^{(i)}$.
To simplify the ratio, we can use \eqref{eq:PG_Xposterior} to write
\begin{equation*}
p( dx_{0:t-1}^{(i)} \mid \theta, y_{0:t-1} )
\propto \mu_\theta(dx_0^{(i)}) g_\theta(y_0 \mid x_0^{(i)}) 
        \prod_{s=1}^{t-1} K_\theta(dx_s^{(i)} \mid x_{s-1}^{(i)}) 
        g_\theta(y_s \mid x_s^{(i)})
\end{equation*}
and
\begin{align*}
p( d(x_{0:t-1}^{(i)}, x_{t:T}^\star) \mid \theta, y_{0:T} )
&\propto \mu_\theta(dx_0^{(i)}) g_\theta(y_0 \mid x_0^{(i)}) 
        \left\{ \prod_{s=1}^{t-1} K_\theta(dx_s^{(i)} \mid x_{s-1}^{(i)})
        g_\theta(y_s \mid x_s^{(i)}) \right\} \\
    &\qquad\times K_\theta(dx_t^\star \mid x_{t-1}^{(i)}) g_\theta(y_t \mid x_t^\star)
        \left\{ \prod_{s=t+1}^T K_\theta(dx_s^\star \mid x_{s-1}^\star)
...
\begin{align*}
\frac{p( d(x_{0:t-1}^{(i)}, x_{t:T}^\star) \mid \theta, y_{0:T} )}
        {p( dx_{0:t-1}^{(i)} \mid \theta, y_{0:t-1} ) }
&\propto K_\theta(dx_t^\star \mid x_{t-1}^{(i)}) g_\theta(y_t \mid x_t^\star) 
        \prod_{s=t}^T K_\theta(dx_s^\star \mid x_{s-1}^\star) 
        g_\theta(y_s \mid x_s^\star) \\
&\propto K_\theta(dx_t^\star \mid x_{t-1}^{(i)}) .
\end{align*}
...
This will rule out ancestor sampling in some applications.
