...
\caption[Ancestral degeneracy]{Illustration of ancestral degeneracy and the mitigating effect of low-variance and adpative resampling. \subref{fig:ancdegen_mn} with multinomial resampling, \subref{fig:ancdegen_adaptsyst} the same system with adaptive systematic resampling.}
...
As usual, $\mu^\theta$, $(K_t^\theta)$, $(g_t^\theta)$ and $p$ are part of the model and are assumed to be known but not necessarily tractable.
...
\For{$t \in \{0,\dots, T-1\}$}{
	...
	\lFor{$i \in \{1,\dots,N\}$}{
		Sample $X_{t+1}^{(i)} \sim K_{t+1}^\theta( \cdot \mid X_t^{(a_t^{(i)})} )$
	}
    	\lFor{$i \in \{1,\dots, N\}$}{	
		$w_{t+1}^{(i)} \gets \Big\{ {\sum_{j=1}^N g_{t+1}^\theta(y_{t+1} \mid X_{t+1}^{(j)}) }\Big\}^{-1} g_{t+1}^\theta(y_{t+1} \mid X_{t+1}^{(i)}) $
	}          
}
\vspace*{10pt}
...
\For{$t \in \{0,\dots, T-1\}$}{
	...
	Sample $a_t^{(a_{t+1}^\star)} \sim \Cat\left( \{1,\dots,N\}, w_t^{(1:N)} q_{t+1}^\theta(x_{t+1}^\star \mid X_{t}^{(1:N)}) \right)$\;
        Sample $a_t^{(1:N)}\setminus a_t^\star \sim \textsc{resample}(\{1,\dots,N\}, w_t^{(1:N)})$\;
	\lFor{$i \in \{1,\dots,N\}$}{
		Sample $X_{t+1}^{(i)} \sim q_{t+1}^\theta( \cdot \mid X_t^{(a_t^{(i)})} )$
	}
    	\lFor{$i \in \{1,\dots, N\}$}{	
		$w_{t+1}^{(i)} \gets \Big\{ {\sum_{j=1}^N g_{t+1}^\theta( y_{t+1} \mid X_{t+1}^{(j)}) }\Big\}^{-1} g_{t+1}^\theta( t_{t+1} \mid X_{t+1}^{(i)}) $
	}          
}
\vspace*{10pt}
...
\end{algorithm}
Now assume that the smoothing distributions admit densities, that is $\mu^\theta(\cdot)$ and $K_t^\theta(\cdot\mid x)$, for any $x,t$, admit densities. Denote the density of $K_t^\theta$ by $q_t^\theta$. Define the trajectories $X_{s, 0:s}^{(i)}$ (for any $s,i$) as before, by tracing back the states of the parents starting from $X_{s,s}^{(i)} := X_s^{(i)}$.
...
\end{cases}
\end{equation}
Therefore, in order to do ancestor sampling, we need a stronger assumption on the Markov kernels than was required to simply run the conditional SMC algorithm: we now require that, for each $t$, $K_t^\theta$ admits a density $q_t^\theta$ and that $q_t^\theta(\cdot \mid x)$ can be evaluated pointwise for any $x$, whereas previously we only needed to draw samples from $K_t^\theta(\cdot\mid x)$ for any $x$.
This additional requirement rules out ancestor sampling in some applications.

Recall that the usual backward simulation procedure requires a full forward pass to calculate the future states before the backward simulation probabilities can be computed. Ancestor sampling, on the other hand, does not require a forward pass because it only computes backward simulation probabilities for the immortal trajectory, for which all the future states are known in advance. This means that ancestor sampling can be implemented on-line.
