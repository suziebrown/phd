...
\draft{This one doesn't fit under the SRs umbrella as previously claimed --- oops! Proof for this one is in progress, but shouldn't be too difficult. Like SRs, there are only finitely many possible counts conditional on weights, so the same kind of proof will work (now with three cases).}
