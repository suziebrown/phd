...
\seb{I have proofs for the next three lemmata, I'm just working on a presentation that might be intelligible to someone other than myself.}
...
%%% The below definition will instead appear in the proof of this Lemma; it is no longer needed in the statement.
...
\begin{lemma}\label{thm:indicators_general}
...
To see that this choice of $f$ satisfies the conditions of Lemma \ref{thm:kjjslemma2}, note that 
\begin{equation}
\sum_{r=1}^{\tau_N(s)} \1{\{c_N(r) \geq K \}} \leq \sum_{r=1}^{\tau_N(s)} \frac{c_N(r)}{K} \leq \frac{s+1}{K} < \infty .
\end{equation}
...
\end{equation}
and the result follows by applying Lemma~\ref{thm:indicators_general} with $K=t/l$.
\end{proof}
...
\end{equation}
\end{lemma}
\seb{Proof will be courtesy of Jere's note with a few edits and probably more explicit workings.}
...
Fix $0 < \varepsilon < (n-3)(n-4)$ and let $N > \max\{ \varepsilon^{-1}, (\binom{n-2}{2} - 2\varepsilon)^{-1} \}$. For each $r,i$ define the event $A_i(r) := \{ \nu_r^{(i)} \leq N\varepsilon \}$.
...
...
\seb{The rest of the proof follows the rest of Jere's note...}
\end{proof}
