...
\left\{ - \alpha (1+O(N^{-1})) \left[ c_N(r_j) + B_n D_N(r_j) \right] \right\} \right)
\times \prod_{r=1}^{\tau_N(t)} \1{E_r} \notag\\
&= \prod_{r=1}^{\tau_N(t)} \1{E_r} 
+ \sum_{k=1}^{\tau_N(t)} \left\{- \alpha (1+O(N^{-1}))\right\}^k 
...
&\qquad + \E \left[ \sum_{k=1}^{\infty} \left\{- \alpha (1+O(N^{-1}))\right\}^k \I{k \leq \tau_N(t)} \1{\bigcap E_r} \sum_{\substack{r_1<\dots<r_k \\ =1}}^{\tau_N(t)}\prod_{j=1}^k 
\left\{ c_N(r_j) + B_n D_N(r_j) \right\} \right] \notag\\
&= \Prob \left[ \bigcap_{r=1}^{\tau_N(t)} E_r \right] \notag\\
&\qquad + \sum_{k=1}^{\infty} \left\{- \alpha (1+O(N^{-1}))\right\}^k
\E \left[ \sum_{\substack{r_1<\dots<r_k \\ =1}}^{\tau_N(t)}\prod_{j=1}^k 
...
Then, since the conditioning can only decrease the values of $c_N(s)$,
...
E_t := \left\{ c_N(t) < \frac{(N-3)_{n-3}}{N^{n-3}} \left( \alpha \left(1 + \frac{2}{N-2} \right) + B_n \right)^{-1} \right\} 
\end{equation}
where $\alpha$ and $B_n$ are positive constants as in \eqref{eq:9a}.
Then, for all $t>0$,
\begin{equation}
...
\end{align}
where the inequality holds by considering the two possible values of $\1{\bigcup E_r^c}$, and the second line follows from \citet[Lemma 2]{koskela2018} when the function $c_N(r)$ is replaced by $\1{E_r^c}$.
Using the generalised Markov inequality,
\begin{align}
\Prob [E_r^c \mid \mathcal{F}_{r-1} ] &= \Prob \left[ c_N(r) \geq \frac{(N-3)_{n-3}}{N^{n-3}} \left( \alpha \left(1 + \frac{2}{N-2} \right) + B_n \right)^{-1} \middle| \mathcal{F}_{r-1} \right] \notag\\
&\leq \E[c_N(r)^2 \mid \mathcal{F}_{r-1} ] \frac{N^{2(n-3)}}{(N-3)_{n-3}^2} \left( \alpha \left(1 + \frac{2}{N-2} \right) + B_n \right)^2 .
\end{align}
Now, using \citet[Lemma 2]{koskela2018} again, but with $c_N(r)$ replaced by $c_N(r)^2$,
\begin{align}
\Prob \left[  \bigcap_{r=1}^{\tau_N(t)} E_r \right]
&\geq 1 - \E \left[  \sum_{r=1}^{\tau_N(t)} \E[c_N(r)^2 \mid \mathcal{F}_{r-1} ] \frac{N^{2(n-3)}}{(N-3)_{n-3}^2} \left( \alpha \left(1 + \frac{2}{N-2} \right) + B_n \right)^2 \right] \notag\\
...
\begin{lemma}
For all $k\geq 1$, for all $t>0$,
\begin{equation}
\lim_{N\to\infty} \Prob \left[k \leq \tau_N(t), \bigcap_{r=1}^{\tau_N(t)} E_r \right] = 1 .
\end{equation}
\end{lemma}
\begin{proof}
We will construct a constant $C_1$ such that 
...
\end{equation}
Any $C_1 \leq \frac{(N-3)_{n-3}}{N^{n-3}} \left( \alpha \left(1 + \frac{2}{N-2} \right) + B_n \right)^{-1}$ will be sufficient for $\bigcap_{r=1}^{\tau_N(t)} E_r$.
...
\begin{equation}
C_1 = \min \left\{ \frac{(N-3)_{n-3}}{N^{n-3}} \left( \alpha \left(1 + \frac{2}{N-2} \right) + B_n \right)^{-1} ,\, \frac{t}{k} \right\} .
\end{equation}
Noting that $t$ is fixed, it can be shown that \eqref{eq:39} is satisfied by modifying the constant in the proof of Lemma \ref{thm:indicators_prob1}.
\end{proof}
