...
\section{An existing limit theorem \seb{$\checkmark$} }
\seb{Say somewhere that we can only get pair-merger-only limits because we take a sparse sample; obviously SMC resampling can induce $>$pair mergers.}
...
\subsection{Proof of theorem, part two \seb{$\checkmark$}}
\seb{Update KJJS references to point to relevant places in the erratum.}
...
A slight modification of the argument allows a similar lower bound to be obtained via Proposition~\ref{thm:pDelta_LB} such that as $N\to\infty$ the bound coincides with the corresponding finite-dimensional distribution of the $n$-coalescent, as required.
...
At the remaining times $r$ the partition is unchanged, and the bound in Proposition~\ref{thm:pDelta_LB} has been applied to the one-step transition probabilities corresponding to these identity transitions. 
The constant $\tilde{B}_n := B_n\binom{n}{2}$ where $B_n$ is the constant defined in Proposition~\ref{thm:pDelta_LB}, and we have replaced $|\eta_d|$ by its upper bound $n$.
...
A multinomial expansion of the product on the second line yields\seb{(noting that $\ON$ raised to any power is $\ON$)}
\begin{align*}
\chi_d 
...
&\geq \sum_{ \beta_1 = 0 }^{ \infty } \ldots \sum_{ \beta_k = 0 }^{ \infty } 
        ...
        \Bigg\} 
\end{align*}
where the expectation of the indicators converges to 1 due to \textcite[Equation (16)]{koskela2018}\seb{and Lemma 4 in the appendix of \texttt{full\_redraft.pdf}}.
\end{proof}
