...
plot()
plot!(nvals, mean4exc, ribbon=(l4exc, u4exc), label="MAP+4SD:exc", marker=mymarkers[5], markerstrokewidth=0, seriescolor=:auto,
    xaxis=:log10, title="including/excluding immortal particle: N=$N, nrep=$nrep, nsd=4", titlefontsize=10,
    xlabel="n", ylabel="tree height /N", leg=:topleft)
plot!(nvals[noob4inc.==0], mean4inc[noob4inc.==0], ribbon=(l4inc[noob4inc.==0], u4inc[noob4inc.==0]), label="MAP+4SD:inc", marker=mymarkers[5], markerstrokewidth=0, seriescolor=:auto, line=:dash)

plot()
plot!(nvals, mean3exc, ribbon=(l3exc, u3exc), label="MAP+3SD:exc", marker=mymarkers[4], markerstrokewidth=0, seriescolor=:auto,
    xaxis=:log10, title="including/excluding immortal particle: N=$N, nrep=$nrep, nsd=3", titlefontsize=10,
    xlabel="n", ylabel="tree height /N", leg=:topleft)
plot!(nvals, mean3inc, ribbon=(l3inc, u3inc), label="MAP+3SD:inc", marker=mymarkers[4], markerstrokewidth=0, seriescolor=:auto, line=:dash)

plot()
plot!(nvals, mean2exc, ribbon=(l2exc, u2exc), label="MAP+2SD:exc", marker=mymarkers[3], markerstrokewidth=0, seriescolor=:auto,
    xaxis=:log10, title="including/excluding immortal particle: N=$N, nrep=$nrep, nsd=2", titlefontsize=10,
    xlabel="n", ylabel="tree height /N", leg=:topleft)
plot!(nvals, mean2inc, ribbon=(l2inc, u2inc), label="MAP+2SD:inc", marker=mymarkers[3], markerstrokewidth=0, seriescolor=:auto, line=:dash)

plot()
plot!(nvals, mean1exc, ribbon=(l1exc, u1exc), label="MAP+1SD:exc", marker=mymarkers[2], markerstrokewidth=0, seriescolor=:auto,
    xaxis=:log10, title="including/excluding immortal particle: N=$N, nrep=$nrep, nsd=1", titlefontsize=10,
    xlabel="n", ylabel="tree height /N", leg=:topleft)
plot!(nvals, mean1inc, ribbon=(l1inc, u1inc), label="MAP+1SD:inc", marker=mymarkers[2], markerstrokewidth=0, seriescolor=:auto, line=:dash)

plot()
plot!(nvals, mean0exc, ribbon=(l0exc, u0exc), label="MAP+0SD:exc", marker=mymarkers[1], markerstrokewidth=0, seriescolor=:auto,
    xaxis=:log10, title="including/excluding immortal particle: N=$N, nrep=$nrep, nsd=1", titlefontsize=10,
    xlabel="n", ylabel="tree height /N", leg=:topleft)
plot!(nvals, mean0inc, ribbon=(l0inc, u0inc), label="MAP+0SD:inc", marker=mymarkers[1], markerstrokewidth=0, seriescolor=:auto, line=:dash)
...
#savefig("CSMC_treeheight_incexc_nsd0.pdf")
