...
\hline\hline
& \thead{support of $\nu_t^{(i)}$ given\\ $ k/N \leq w_t^{(i)} < (k+1)/N$} 
        & \thead{stochastic\\ rounding?}       
        & \thead{degenerate if\\ $w_t^{(1:N)} =$\\ $(1,\dots,1)/N$?} 
        & \thead{sensitive to\\ permutations\\ of weights?} 
        & \thead{PRNG\\ calls} \\
\hline
\texttt{multi} & $\{0,\dots,N\}$ & $\times$ & $\times$ & $\times$ & $N$ \\
\texttt{star} & $\{0, N\}$ & $\times$ & $\times$ 
        & $\times$ & $1$ \\
\texttt{strat} & $\{k-1, k, k+1, k+2\}$ & $\times$ & $\checkmark$ 
        & $\checkmark$ & $N$ \\
\texttt{syst} & $\{k, k+1\}$ & $\checkmark$ & $\checkmark$ 
        & $\checkmark$ & $1$ \\
\texttt{res-multi} & $\{k,\dots,N\}$ & $\times$ & $\checkmark$ 
        & $\times$ & $\leq N$ \\
\texttt{res-star} & $\{k, N\}$ & $\times$ & $\checkmark$ 
        & $\times$ & $1$ \\
\texttt{res-strat} & $\{k, k+1, k+2\}$ & $\times$ & $\checkmark$ 
        & $\checkmark$ & $\leq N$ \\
\texttt{res-syst} & $\{k, k+1\}$ & $\checkmark$ & $\checkmark$ 
        & $\checkmark$ & $1$ \\
\texttt{ssp} & $\{k, k+1\}$? & $\checkmark$? & $\checkmark$? & $\checkmark$? & ? \\
\texttt{branching} & $\{k, k+1\}$? & $\checkmark$? & $\checkmark$? & $\checkmark$? & ? \\
\hline\hline
\end{tabular}
\caption{Properties of resampling schemes}
\label{tab:resampling_properties}
\end{table} 
 
 
\subsubsection{Multinomial resampling}%\label{sec:resampling_multinomial}
...
Fast implementations of multinomial resampling rely on the $U_i$s being sorted, which speeds up the search step \eqref{eq:syst_strat_resampling}. Sorting $N$ numbers is an $O(N\log N)$ operation, but in fact we can directly sample the order statistics of a $\Unif[0,1]$ distribution \seb{[citations: \textcite{chopin2020}, and a different (or possibly equivalent) method in \textcite{hol2006}] ---explore whether these methods are equivalent}.
...
\subsubsection{Residual resampling}%\label{sec:resampling_residual}
...
Each particle $X_{t}^{(i)}$ is deterministically assigned $\flnw$ offspring and the remaining $R := N- \sum_{i=1}^N \flnw$ offspring are assigned stichastically according to the residual weights
\begin{equation*}
r^{(i)} := ( Nw_t^{(i)} - \flnw) /R .
\end{equation*}
...
\subsubsection{Stratified resampling}%\label{sec:resampling_stratified}
...
\subsubsection{Systematic resampling}%\label{sec:resampling_systematic}
...
\subsubsection{Star resampling}%\label{sec:resampling_star}
...
\seb{This section was dumped from elsewhere and most of its subsections need redrafting.}

\subsubsection{Variance}
Some results comparing the Monte Carlo variance associated with different resampling schemes were presented in \textcite{douc2005}. 
Their results, plus some others, are presented in Proposition~\ref{thm:resampling_var_compare}.
...
Of direct interest for the performance of the SMC algorithm is the one-step Monte Carlo variance induced by resampling, that is
...
%Proposition~\ref{thm:resampling_var_compare} compares this variance across different resampling schemes.
...
\end{enumerate}
\end{prop}
\draft{Say which parts were proved in \textcite{douc2005} or not.}
...
&= \frac{1}{N} \sum_{j=1}^N \varphi^2(X_t^{(j)}) w_t^{(j)}
        - \frac{1}{N^2} \sum_{j=1}^N \varphi^2(X_t^{(j)}) \flnw
        - \frac{R}{N^2} \left\{ \sum_{j=1}^N \varphi(X_t^{(j)}) r^{(j)} \right\}^2 . 
\end{align*}
By a similar argument, it can be shown that \seb{(using the star resampling part below, which should probably be moved to above here...)}
\begin{equation}
\rho_{\texttt{res-star}}(\varphi)
= R \rho_{\texttt{res-multi}}(\varphi)
\geq \rho_{\texttt{res-multi}}(\varphi) ,
\end{equation}
proving the first inequality in \ref{item:resampling_var4}.

\textbf{star resampling:} all of the $\tilde{X}_t^{(i)}$s are equal, say $\tilde{X}_t^{(1)} = \dots = \tilde{X}_t^{(N)} = X_t^\star$, so
\begin{align*}
\rho_{\texttt{star}}(\varphi)
&= \V\left[ \frac{1}{N} \sum_{i=1}^N \varphi(\tilde{X}_t^{(i)}) \midd \mathcal{G}_t \right]
...
&= N \rho_{\texttt{multi}}(\varphi) .
\end{align*}
This proves part \ref{item:resampling_var3}.

...
\end{proof}

\draft{Can't do a formal variance comparison for syst (and others?).}
...
From this point-of-view, stratified and systematic resampling are in fact quasi-Monte Carlo versions of multinomial resampling.
%We presented multinomial sampling via its inversion sampling implementation. Quasi-Monte Carlo aims to reduce the variance of Monte Carlo estimates by using low-discrepancy points instead of i.i.d. Uniform points for inversion sampling. Stratified and systematic resampling do just that, providing a more regular set of points to feed into the inversion sampler.

\draft{Calculate/compare discrepancy for each mn/strat/syst. Can't achieve minimal discrepancy $1/(2N)$ because we need marginally uniform points for unbiasedness (so really this is RQMC).}
...
\seb{Even \texttt{star} and \texttt{SSP} and \texttt{branching}? If it turns out to differ depending on resampling scheme then include it as a column in Table~\ref{tab:resampling_properties}.}
...
\seb{Properties shared by all SRs: support of offspring counts, degeneracy under equal weights, others?}
