...
\end{equation}
Any $C_1 \leq \frac{(N-3)_{n-3}}{N^{n-3}} \left( \alpha_n \left(1 + \frac{2}{N-2} \right) + B_n \right)^{-1}$ will be sufficient for $\bigcap_{r=1}^{\tau_N(t)} E_r$.
...
\end{align}
Clearly $\Prob \left[k \leq \tau_N(t), \bigcap_{r=1}^{\tau_N(t)} E_r \right] \leq \Prob \left[\bigcap_{r=1}^{\tau_N(t)} E_r \right]$, so the second part of \eqref{eq:37} follows as an immediate corollary.
\end{proof}
...
Let $(\mathcal{F}_t)$ be the filtration generated by the offspring counts $\nu_t^{(1:N)}$ at each generation $t$.
Let $f(t)$ be any $\mathcal{F}_t$-measurable function. %%% not sure measurability is even necessary
Then
\begin{equation}
