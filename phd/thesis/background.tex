...
Considering the illustration Figure~\ref{fig:resampling_stratified}, the distribution of $\nu_t^{(i)}$ depends not only on $w_t^{(i)}$ but also on where the $i^{th}$ weight interval falls with respect to the length-$(1/N)$ intervals. There are two cases to consider, which are illustrated in Figure~\ref{fig:strat_cases}.
The distinction between Cases \subref{fig:strat_case1} and \subref{fig:strat_case2} is whether the ``overhang'' is less than or greater than $1/N$. Arrangements such that one or both ends have no overhang are included in Case \subref{fig:strat_case1}, with the relevant overhang lengths ($\delta_L$ and/or $\delta_R$) set to zero.
Note that Case \subref{fig:strat_case2} cannot happen if $k=0$.
...
To define a probability distribution over these four values, we introduce the notation $p_j := \Prob[ \nu_t^{(i)} = \flnw +j \mid w_t^{(i)} ]$, for $j=-1,0,1,2$.\seb{Change notation to avoid confusion with $p_k^{(i)}$'s in Chapter~\ref{ch:appl}. Also change $\delta$ notation to avoid confusion with Chapter~\ref{ch:appl}.}
