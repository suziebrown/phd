...
\begin{enumerate}
\item \label{item:relcomp1} For every $\varepsilon>0$, $t\geq 0$ there exists a compact set $\Gamma \subseteq \mathcal{P}_n$ such that\seb{again, use $\epsilon$ here and in the following to avoid confusion with $\varepsilon$ of Chapter 4.}
\begin{equation*}
...
\end{equation*}
using the series expansion for the Erlang CDF. \seb{citation?}
Hence 
\begin{equation*}
\liminf_{N\to\infty} 
        \Prob[ T_m^{(N)} \geq t, \varpi_i^{(N)} > \delta \,\forall i\in \{1,\dots,m\} ]
...
\seb{Also include the UB and LB on $p_{\Delta\Delta}$ here (refer to the bounds on $p_{\xi\xi}$ in previous chapter, plugging in $\xi=\Delta$). They are referenced a few times in this section as \eqref{eq:pDelta_UB} and \eqref{eq:pDelta_LB}.}
