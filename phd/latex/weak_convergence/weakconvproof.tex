...
\left\{ c_N(r_j) + B_n D_N(r_j) \right\} 
\middle| k\leq\tau_N(t), \bigcap_{r=1}^{\tau_N(t)} E_r \right] \notag\\
&\leq \frac{1}{k!} \lim_{N\to\infty} \E \left[ \left\{ t + c_N(\tau_N(t)) \right\}^k
\middle| k\leq\tau_N(t), \bigcap_{r=1}^{\tau_N(t)} E_r \right] \notag\\
&\qquad+ \frac{1}{k!}
\sum_{I=0}^{k-1}  \binom{k}{I}
( B_n )^{k-I}
(t+1)^{k-1}
\lim_{N\to\infty} \E\left[ \sum_{r=1}^{\tau_N(t)} D_N(r) \middle| k\leq\tau_N(t), \bigcap_{r=1}^{\tau_N(t)} E_r \right] \notag\\
&\leq \frac{1}{k!} \lim_{N\to\infty} \E \left[ \left\{ t + c_N(\tau_N(t)) \right\}^k \right] \notag
+ \frac{1}{k!}
\sum_{I=0}^{k-1}  \binom{k}{I}
( B_n )^{k-I}
(t+1)^{k-1}
\lim_{N\to\infty} \E\left[ \sum_{r=1}^{\tau_N(t)} D_N(r) \right] \notag\\
&= \frac{1}{k!} t^k 
\end{align}
again using that the conditioning can only decrease the expectations.
