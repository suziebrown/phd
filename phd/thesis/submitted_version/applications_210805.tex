...
For instance, we have generally required that the resampling scheme is unbiased, but this is by no means necessary for Theorem~\ref{thm:weakconv} (or indeed Theorem~\ref{thm:FDDconv}); it is just that biased resampling schemes are of little use in SMC.
...
For many of the following results it will be necessary to compute filtered expectations $\Et[\cdot] \equiv \E[\cdot \mid \mathcal{F}_{t-1}]$, which are generally difficult to compute directly. 
...
\end{equation}
at each time $t$ forms a separatrix, in the sense of d-separation \parencite{verma1988}, between the parental indices $a_t^{(1:N)}$ and the previous $\sigma$-algebra $\mathcal{F}_{t-1}$ in the filtration. 
...
Consider an SMC algorithm using multinomial resampling, such that \textup{\ref{standing_assumption}} is satisfied. Assume there exist constants $\varepsilon\in (0,1], a\in [1,\infty)$ and probability density $h$ such that for all $x, x^\prime, t$,
...
\end{equation}
Let $(G_t^{(n,N)})_{t\geq0}$ denote the genealogy of a random sample of size $n$ among the $N$ terminal particles in the output of the algorithm. Then, for any fixed $n$, the time-scaled genealogy $(G_{\tau_N(t)}^{(n,N)})_{t\geq0}$ converges weakly to Kingman's $n$-coalescent as $N\to \infty$.%, in the sense of finite-dimensional distributions.
\end{corollary}
...
\Prob \left[ a_t^{(i)} = a_i \midd \mathcal{H}_t \right] 
...
\begin{equation*}
\Prob \left[ a_t^{(1:N)} = a_{1:N} \midd \mathcal{H}_t \right] 
...
For any $k\in\mathbb{N}$ the function $a_t^{(1:N)} \to (\nu_t^{(i)})_k$ is $\{i\}$-increasing in the sense of \textcite[p.19]{koskela2018}, so we may apply the bounds
\begin{equation*}
\E\left[ (V_L^{(i)})_k \right]
\leq \E\left[ (\nu_t^{(i)})_k \midd \mathcal{H}_t \right]
\leq \E\left[ (V_U^{(i)})_k \right] ,
\end{equation*}
where
\begin{align*}
& V_L^{(i)} 
\eqdist \Bin\left(N, \frac{\varepsilon/a}{(\varepsilon/a) + (N-1)(a/\varepsilon)} \right),\\
& V_U^{(i)} 
\eqdist \Bin\left( N, \frac{a/\varepsilon}{(a/\varepsilon) + (N-1)(\varepsilon/a)} \right) .
\end{align*}
...
\begin{equation}
\frac{1}{(N)_2} \sum_{i=1}^N \E\left[ (\nu_t^{(i)})_2 \midd \mathcal{H}_t \right]
\geq \frac{\varepsilon^4}{Na^4} \label{eq:mn_cN_LB}
\end{equation}
and
\begin{equation}
\frac{1}{(N)_3} \sum_{i=1}^N \E\left[ (\nu_t^{(i)})_3 \midd \mathcal{H}_t \right]
...
\end{align*}
Thus \eqref{eq:mainthmcondition2} is satisfied. 
...
Consider an SMC algorithm using multinomial resampling, satisfying \textup{\ref{standing_assumption}} and \eqref{eq:gq_bounds_mn}. 
...
Consider an SMC algorithm using stratified resampling, such that \textup{\ref{standing_assumption}} is satisfied.
...
&= \flnw \E\left[ (\nu_t^{(i)})_2 \midd \mathcal{H}_t \right] \notag\\
&\leq a^2 \E\left[ (\nu_t^{(i)})_2 \midd \mathcal{H}_t \right] . \label{eq:strat_v2_versus_v3}
\end{align}
...
Some terms in the above expressions may be equal to zero when $w_t^{(i)}$ is small enough, but the bound still holds in these cases.
Since \eqref{eq:strat_v2_versus_v3} holds for all $i$, applying the tower rule as in \eqref{eq:condexp_HtFt} we have
\begin{equation*}
\frac{1}{(N)_3} \sum_{i=1}^{N} \Et\left[ (\nu_t^{(i)})_3 \right]
\leq \frac{a^2}{N-2} \frac{1}{(N)_2} \sum_{i=1}^{N} \Et\left[ (\nu_t^{(i)})_2 \right] ,
\end{equation*}
satisfying \eqref{eq:mainthmcondition2} with $b_N := a^2/(N-2) \rightarrow 0$.
...
\begin{equation}
\Prob\left[ \max_i w_t^{(i)} - \min_i w_t^{(i)} \geq 2\delta/N \midd \mathcal{F}_{t-1} \right] \geq \zeta \label{eq:strat_minmaxweight}
\end{equation}
...
In fact, the stated condition is stronger than necessary: we only need the bounds on $q_t$ to hold for infinitely many $t$, rather than for all $t$. We use this stronger statement to avoid complicating the proof.
...
\label{sec:convergenceSRs}

\begin{corollary}\label{thm:stochrounding}
Consider an SMC algorithm using any stochastic rounding as its resampling scheme, such that \textup{\ref{standing_assumption}} is satisfied.
Assume that there exists a constant $a\in [1,\infty)$ such that for all $x, x^\prime, t$,
\begin{equation*}
\frac{1}{a} \leq g_t(x, x^\prime) \leq a . 
\end{equation*}
...
uniformly in $x,t$, and that there exist $\zeta >0$ and $\delta \in (0,1)$ such that 
\begin{equation*}
\Prob\left[ \max_i w_t^{(i)} - \min_i w_t^{(i)} \geq 2\delta/N \midd \mathcal{F}_{t-1} \right] \geq \zeta
\end{equation*}
 for infinitely many $t$. Then, for all $N>1$, $\Prob[ \tau_N(t) = \infty ] =0$ for all finite $t$.
\end{prop}
...
\begin{align*}
&\Prob\left[ \nu_t^{(i)} = \flnw \midd w_t^{(1:N)} \right]
= 1- N w_t^{(i)} + \flnw , \\
&\Prob\left[ \nu_t^{(i)} = \flnw +1 \midd w_t^{(1:N)} \right]
= N w_t^{(i)} - \flnw .
\end{align*}
...
\begin{equation*}
\Prob\left[ c_N(t) > 2/N^2 \midd \mathcal{H}_t \right]
\geq 1- \Prob\left[ \nu_t^{(i^\star)} =1 \midd \mathcal{H}_t \right]
= \begin{cases}
    1 - p_0^{(i^\star)} & \text{if } \gamma \in [0,1) \\
...
\begin{equation*}
\Prob\left[ c_N(t) > 2/N^2 \midd \mathcal{H}_t \right]
\geq 1- \Prob\left[ \nu_t^{(j^\star)} =1 \midd \mathcal{H}_t \right]
= \begin{cases}
    0 & \text{if } \gamma = 0 \\
    1 - p_1^{(j^\star)} & \text{if } \gamma \in (0,1) \\
    1 & \text{if } \gamma = 1 .
...
Consider an SMC algorithm using residual resampling with stratified residuals, such that \textup{\ref{standing_assumption}} is satisfied.
Assume that there exists a constant $a\in [1,\infty)$ such that for all $x, x^\prime, t$,
\begin{equation*}
\frac{1}{a} \leq g_t(x, x^\prime) \leq a .
\end{equation*}
...
\geq p_1^{(i^\star)} 
\geq \Prob\left[ \nu_t^{(i^\star)} = \flnw[i^\star] + 1 \midd w_t^{(1:N)} \right] \varepsilon^{2N}
\end{equation*}
...
\geq p_0^{(j^\star)} 
\geq \Prob\left[ \nu_t^{(j^\star)} = \flnw[j^\star] \midd w_t^{(1:N)} \right] \varepsilon^{2N} 
\end{equation*}
by \eqref{eq:strat_pbounds}.
...
One might ask the question: is it possible to construct an SMC algorithm whose genealogies converge to some object other than the $n$-coalescent?
...
It is easy enough to show that such a resampling scheme does not satisfy \eqref{eq:mainthmcondition2}.
...
Now we know that Theorem~\ref{thm:FDDconv} does not apply, but this is not enough because condition \eqref{eq:mainthmcondition2} was not proved to be necessary.
...
Consider a conditional SMC algorithm using multinomial resampling, such that \textup{\ref{standing_assumption}} is satisfied. Assume there exist constants $\varepsilon\in (0,1]$ and $a\in [1,\infty)$ and probability density $h$ such that for all $x, x^\prime, t$,
...
Define $\mathcal{H}_t$ as in \eqref{eq:defn_Ht}.
The parental indices are conditionally independent given $\mathcal{H}_t$, as in standard SMC with multinomial resampling, but we have to treat $i=1$ as a special case. The conditional law on the $i^\text{th}$ parental index is
\begin{equation*}
\Prob \left[ a_t^{(i)} = a_i \midd \mathcal{H}_t \right] \propto
\begin{cases}
\1{a_i=1} &i=1 \\
...
\begin{align}
\frac{1}{(N)_2} \sum_{i=1}^N \E\left[ (\nu_t^{(i)})_2 \midd \mathcal{H}_t \right]
&\geq \frac{1}{(N)_2} \sum_{i=1}^N \E\left[ ( V_L^{(i)} )_2 \right]
...
\begin{align*}
\frac{1}{(N)_3} \sum_{i=1}^N \E\left[ (\nu_t^{(i)})_3 \midd \mathcal{H}_t \right]
&\leq \frac{1}{(N)_3} \sum_{i=1}^N \E\left[ ( V_U^{(i)} )_3 \right]
...
\end{equation*}
so \eqref{eq:mainthmcondition2} is satisfied. 
...
Consider a conditional SMC algorithm using multinomial resampling, satisfying \textup{\ref{standing_assumption}} and \eqref{eq:gq_bounds_csmc}. 
Then, for all $N>2$, $\Prob[ \tau_N(t) = \infty ]=0$ for all finite $t$.
\end{lemma}
