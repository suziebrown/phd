...
At every resampling step, some parents may be assigned more than one offspring each, so the further back in time you look, the more of the ancestries of the terminal particles have coalesced (see Figure~\ref{fig:ancestral_degeneracy_mn}).
...
\draft{Adaptive resampling: when is it effective or not? Backward sampling: when is it possible to do this?}

What can we do about it? There are a few possible approaches.

Firstly, we could try to reduce the number of offspring being assigned to any one parent in each resampling step. We can only go so far, because we need the resampling procedure to remain unbiased (Section~??), but we can try to reduce the variance inherent in the resampling procedure. This idea, known as \emph{low-variance resampling}\seb{[citation?]} is discussed in detail in Section~\ref{sec:resampling}.
...
Adaptive resampling can also be combined with low-variance resampling. 
Figure~\ref{fig:ancestral_degeneracy} compares the ancestral degeneration under multinomial resampling (which has fairly high variance) versus the same under adaptive resampling with a minimum-variance resampling scheme.
Clearly the degeneration is much more severe in the former case.
