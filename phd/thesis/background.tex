...
\draft{Tour of the key resampling schemes (multinomial, residual-*, stratified, systematic, and the worst possible scheme).}\\
...
\end{equation*}
\seb{I'm not sure $\mathcal{F}$ is the right filtration; we need to condition on positions etc. as well as offspring counts for the calculations to work? And don't we want a forward-in-time filtration to calculate the one-step variance? Use $\mathcal{G}_t$ as in DCM05 and add it to the notation list too.}
