...
The standing assumption is a weaker condition than exchangeability of the particles within a generation, which is sufficient to admit an exchangeable process in the limit.
Exchangeability of the particles would imply neutrality, an unreasonable assumption in the setting of SMC. 
In contrast, the standing assumption can easily be enforced upon any SMC algorithm by applying a random permutation to the offspring indices immediately after resampling.
...
\draw[->, thick] (0.6,0)-- node[sloped, anchor=center, text width=2cm] 
        {\footnotesize{Brown~et~al.~2021 Lemma 3.4}} (3.5,-2);
\end{tikzpicture}
...
\draft{Explain relationship between this and KJJS theorem, e.g.\ which conditions are implied by each other (with a diagram?!). Proof that KJJS conditions are implied by ours. Modification of KJJS proof (or even write out a complete proof?) using weaker bound on $p_{\xi\xi}$ (that bound should have been stated and proved already in transition probabilities section).}
\seb{Still need to add the proof of KJJS theorem with changed assumption (sec3.1 in BJJK).}
\seb{Use $\epsilon$ for the proofs to distinguish from $\varepsilon$?}
...
\begin{proof}
Fix $\varepsilon>0$, and assume $N>2/\varepsilon$.
...
&\leq \frac{N\varepsilon}{(N)_2} \sum_{i=1}^N \nu_t^{(i)}
        + \sum_{i=1}^N \1{A_i(t)^c} \\
&= \frac{N\varepsilon}{N-1} + \sum_{i=1}^N \1{A_i(t)^c} .
\end{align*}
Taking expectations and applying the generalised Markov inequality,
\begin{align*}
\E[c_N(t)]
&\leq \varepsilon\ON + \sum_{i=1}^N \Prob[ \nu_t^{(i)} > N\varepsilon ] \\
&\leq \varepsilon\ON 
        + \sum_{i=1}^N \frac{\E[ (\nu_t^{(i)})_3 ]}{(N\varepsilon)_3} \\
&\leq \varepsilon\ON + \frac{N(N)_2}{(N\varepsilon)_3} \E[ D_N(t) ] \\
&= \varepsilon\ON + \varepsilon^{-3}\ON \E[ D_N(t) ] \\
&\leq \varepsilon\ON + \varepsilon^{-3}\ON 
        \E\left[ \sum_{r=1}^t D_N(r) \right] \\
&\leq \varepsilon\ON + \varepsilon^{-3}\ON
        \E\left[ \sum_{r=\tau_N(0)+1}^{\tau_N(t)} D_N(r) \right] .
\end{align*}
Taking limits and using \eqref{eq:kjjs_big_merger_bound}, 
\begin{equation}
\lim_{N\to\infty} \E[c_N(t)] \leq \varepsilon .
\end{equation}
...
\begin{align*}
\E[c_N(\tau_N(t))]
&\leq \varepsilon\ON + \sum_{i=1}^N \Prob[ \nu_{\tau_N(t)}^{(i)} > N\varepsilon ] \\
&\leq \varepsilon\ON + \varepsilon^{-3}\ON \E\left[ D_N(\tau_N(t)) \right] \\
&\leq \varepsilon\ON + \varepsilon^{-3}\ON
        \E\left[ \sum_{r=\tau_N(0)+1}^{\tau_N(t)} D_N(r) \right] \\
&\underset{N\to\infty}{\longrightarrow} \varepsilon
\end{align*}
which concludes the proof.
\end{proof}
...
&\leq \frac{ 1 }{ N^2 ( N )_2 } \sum_{ j=1 }^N \sum_{ i \neq j } 
        ...
        \label{DN_part_2}
\end{align}
Now, with the events $A_i$ defined as in \eqref{eq:define_Ai_events}\seb{should have explicit time-dependence},
\begin{align}
\sum_{ j=1 }^N \sum_{ i \neq j } \Et\{ ( \nu_t^{(i)} )_2 ( \nu_t^{(j)} )_2\} 
&= \sum_{ j=1 }^N \sum_{ i \neq j } 
        \Big\{ \Et[ ( \nu_t^{(i)} )_2 ( \nu_t^{(j)} )_2 \1{ A_i } ]
...
\end{align}
For $N \geq 3 / \varepsilon$, by the generalised Markov inequality,
...
&\leq \{ 1 + O( N^{ -1 } ) \} \frac{ b_N }{ \varepsilon^3 } \Et\{ c_N( t )\}. 
\end{align}
