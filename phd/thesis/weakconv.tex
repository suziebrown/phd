...
Suppose that for some fixed $t>0$, there exist $m \in \mathbb{N}$ and $\delta >0$ such that
...
Indeed $(Z_{\tau_N(\cdot)})$ is constant on every interval $[ T_{i-1}^{(N)} , T_i^{(N)} )$, so $\omega( (Z_{\tau_N(\cdot))} , \delta, t ) = 0$. 
...
&\geq \Prob[ T_m^{(N)} \geq t ]
        \,\Prob[ \varpi_i^{(N)} > \delta \,\forall i\in \{1,\dots,m\} ] .
\end{align}
%Due to Lemma ?? we have that $\tau_1^{(N)}, \dots, \tau_m^{(N)}$ converge jointly in distribution to i.i.d $Exp(\alpha_n t)$- distributed random variables, and hence $T_m^{(N)}$ converges in distribution to a $Gamma(m, \alpha_n t)$-distributed random variable.
Due to Lemma ??, \seb{This lemma should give the limiting distribution of the $\varpi$s.}
\begin{equation}
...
This argument applies for any $\varepsilon$, $t$, so Condition~\ref{item:relcomp2} is satisfied, and the proof is complete.
\end{proof}
