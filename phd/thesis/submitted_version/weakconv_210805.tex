...
%%%%------ ATTEMPT 2, I WASN'T CONVINCED BY THE RIGOUR OF THIS ---------
%Due to Lemma \ref{thm:holdingtimes_distn}, the limiting distributions of $\varpi_i^{(N)}$ are i.i.d. $\Exp(\alpha_n)$, where $\alpha_n := n(n-1)/2$, so
%\begin{equation*}
...
%\end{equation*}
%using the series expansion for the Erlang CDF \parencite[see for example][Chapter 15]{forbes2011}.
%Since $T_m^{(N)} = \varpi_1^{(N)} + \cdots + \varpi_m^{(N)}$, there is a positive correlation between $T_m^{(N)}$ and each of the $\varpi_i^{(N)}$, and in the limit $N\to\infty$ the $\varpi_i^{(N)}$'s are independent, so
%\begin{align*}
%&\liminf_{N\to\infty} 
%        \Prob[ T_m^{(N)} \geq t, \varpi_i^{(N)} > \delta \,\forall i\in \{1,\dots,m\} ] \\
%&\hspace{4cm}\geq \liminf_{N\to\infty} \Prob[ T_m^{(N)} \geq t ] 
%        \Prob[ \varpi_i^{(N)} > \delta \,\forall i\in \{1,\dots,m\} ]\\
%&\hspace{4cm}\geq \liminf_{N\to\infty} \Prob[ T_m^{(N)} \geq t ] 
%        \liminf_{N\to\infty} \Prob[ \varpi_i^{(N)} > \delta \,\forall i\in \{1,\dots,m\} ]\\
%&\hspace{4cm}= (e^{-\alpha_n \delta})^m e^{-\alpha_n t} \sum_{i=0}^{m-1} 
%        \frac{(\alpha_n t)^i}{i!} ,
%\end{align*}
%which can be made $\geq 1-\epsilon$ by taking $m$ sufficiently large and $\delta$ sufficiently small.
%Since this argument applies for any $\epsilon$ and $t$, \eqref{eq:condition2b} and hence Condition~\ref{item:relcomp2} is satisfied, and the proof is complete.
%%%%%----------------------------------------------------------
%%%-------- PREVIOUS ATTEMPT WHERE CONDITIONING DIDN'T WORK ---------
...
%        \varpi_i^{(N)} > \delta \,\forall i\in \{1,\dots,m\} \mid \mathcal{F}_\infty ] \right] \\
...
\end{align*}
The first term on the right-hand side converges to $\Prob[ T_1 \leq t_1, \dots , T_k \leq t_k ]$ by the induction hypothesis, and it remains to show that
\begin{equation*}
\lim_{N\to\infty} 
...
\end{equation*}
as can be seen by considering the multinomial expansion of the right-hand side.
...
\end{equation}
Applying Proposition~\ref{thm:cN_properties}\ref{item:cN_property4} on the right-hand side of \eqref{eq:002} yields the lower bound.
...
\end{align}
where $[l] := \{1,\dots,l\}$. 
%Since we are summing over all permutations of $s_1,\dots,s_l$, the particular partition of $\{1,\dots,l\}$ into $(\mathcal{I},\mathcal{I}^c)$ is not important; only the size of $\mathcal{I}$ matters. We may therefore replace the sum over $\mathcal{I}$ with a sum over $I:= |\mathcal{I}|$ and, without loss of generality, set $\mathcal{I} = \{1,\dots,I\}$. A binomial factor accounts for the number of partitions of a given size:
...
\end{align}
In the second term on the right-hand side, there is always at least one $D_N$ term, so using that $c_N(s) \geq D_N(s)$ (Proposition~\ref{thm:cN_properties}\ref{item:cN_property2}) we can write
\begin{align}
\sum_{I=0}^{l-1}\binom{l}{I} B^{l-I} 
        &\sum_{\substack{ s_1, \dots, s_l =1 \\ \text{all distinct} }}^{\tau_N(t)}
...
When $N \geq 3$, a sufficient condition to ensure that the expression on the right-hand side of \eqref{eq:018} is non-negative is that the event
...
%We obtain another valid lower bound by multiplying the entire RHS of \eqref{eq:pDeltaDelta_LB} by the indicator on this event, so that if $E_N^3$ does not occur we have the trivial lower bound $1-p_t \geq 0$.
Then
\begin{equation*}
1-p_t
...
%keep the bounds non-negative for all $N\geq 3$ so 
...
&\leq \left( 2 + \frac{\alpha_n^2 (t+1)}{2} \right) 
        ...
        \label{eq:034a}
\end{align}
Now let us consider the remaining sum-product on the right-hand side of \eqref{eq:032a}.
...
&\qquad- \left( \sum_{s=1}^{\tau_N(t)} D_N(s) \right)
    ...
    &\hspace{5cm}+ (t+1) \exp[ \alpha_n \ON (t+1) (1+B_n^\prime) ] 
        \Bigg\} .
\end{align*}
Due to \eqref{eq:BJJK_eq3.3}--\eqref{eq:BJJK_eq3.4}, all but the first line on the right-hand side of the above have vanishing expectation, leaving
\begin{align}
\lim_{N\to\infty} \E &\left[ 
        \sum_{\substack{r_1 <\dots< r_k :\\ r_i \leq \tau_N(t_i) \forall i}}
        \left( \prod_{i=1}^k p_{r_i} \right)
...
%in Lemma~\ref{thm:induction_sumprodcN}:
\begin{align*}
\lim_{N\to\infty} \E &\left[ \1{E_N} 
        \sum_{\substack{r_1<\dots<r_k :\\ r_i\leq \tau_N(t_i) \forall i}} 
        \prod_{i=1}^k c_N(r_i) \right] 
...
\end{equation*}
which is sufficient to ensure the $j^\text{th}$ term in the following product is non-negative,
and define 
