...
\end{equation*}
Although the parental indices are chosen independently, but the resulting offspring counts are negatively correlated. \seb{link to GCW19's negative association?}
...
\draft{Add workings/results for strat and res-strat (see sora). Mention negative association $=$ teaser for later, which has to do with covariance between counts rather than marginal variance.}
...
\draft{Also say something about negative association \`a la GCW19.}
