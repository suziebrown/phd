...
\end{align}
In the second line, there is always at least one $D_N$ term, and $c_N(s) \geq D_N(s)$ for all $s$ \citep[p.9]{koskela2018}, so we can write
\begin{align}
...
\end{equation}
and we define $E := \bigcap_{r=1}^{\tau_N(t)} E_r$ and $E^\prime := \bigcap_{r=1}^{\tau_N(t)} E_r^\prime$.
Then
\begin{equation}
1-p_r
= p_{\Delta\Delta}(r) \leq 1 - \alpha_n (1+O(N^{-1})) 
        \left[ c_N(r) - B_n^\prime D_N(r) \right] \1{E \cap E^\prime} .
\end{equation}
...
&\leq 1 + \sum_{l=1}^{\tau_N(t)} (- \alpha_n)^l (1+O(N^{-1})) 
        \frac{1}{l!} \sum_{s_1\neq\dots\neq s_l}^{\tau_N(t)}\prod_{j=1}^l
        \left[ c_N(s_j) - B_n^\prime D_N(s_j) \right] \1{E \cap E^\prime}\notag\\
...
&\leq 1 + \Bigg\{ \sum_{\substack{l=2 \\ \text{even} }}^{\tau_N(t)} 
    ...
    &\qquad- \sum_{\substack{l=1 \\ \text{odd} }}^{\tau_N(t)} 
        ...
        (t+1)^{l-1} (1+B_n^\prime)^l \right] \Bigg\} \1{E \cap E^\prime} \notag\\
&\leq 1 + \Bigg\{ \sum_{\substack{l=2 \\ \text{even} }}^{\tau_N(t)} 
        \alpha_n^l (1+O(N^{-1})) \frac{1}{l!} 
...
&\leq 1+ \sum_{l=1}^{\infty} (-\alpha_n)^l (1+O(N^{-1})) \frac{1}{l!} t^l
        \I{\tau_N(t) \geq l} \1{E \cap E^\prime}
...
&\leq 1+ \sum_{l=1}^{\infty} (-\alpha_n)^l \frac{1}{l!} t^l
        \lim_{N\to\infty} \Prob \left[ \{\tau_N(t) \geq l\} \cap E \cap E^\prime \right]
...
\end{align}
Passing the limit and expectation inside the infinite sum is justified by dominated convergence and Fubini; see Lemma~\ref{thm:DCT_Fubini}\ref{thm:DCTFubini_a}.
...
\end{align}
Again, passing the limit and expectation inside the infinite sum is justified by dominated convergence and Fubini; see Lemma~\ref{thm:DCT_Fubini}\ref{thm:DCTFubini_a}.
...
\begin{proof}
We use the bound on $(1-p_r)$ from \eqref{eq:018} and apply a multinomial expansion, defining as in \eqref{eq:019b} and \eqref{eq:defn_Eprime} respectively the events $E$ and $E^\prime$ which ensure the bounds are non-negative:
\begin{align}
\prod_{\substack{r=1 \\ \notin \{r_1,\dots,r_k\} }}^{\tau_N(t)} (1-p_r)
&\leq \prod_{\substack{r=1 \\ \notin \{r_1,\dots,r_k\} }}^{\tau_N(t)} 
        \left\{ 1 - \alpha_n  (1+O(N^{-1})) [ c_N(r) - B_n^\prime D_N(r) ] 
        \1{E \cap E^\prime} \right\} \notag\\
&= 1 + \sum_{l=1}^{\tau_N(t) -k}
        (-\alpha_n)^l (1+O(N^{-1})) \frac{1}{l!}
        \sum_{\substack{s_1\neq\dots\neq s_l \\ \notin \{r_1,\dots,r_k\} }}
        ^{\tau_N(t)}
...
&= 1 + \sum_{l=1}^{\tau_N(t) -k}
        (-\alpha_n)^l (1+O(N^{-1})) \frac{1}{l!}
        \sum_{\substack{s_1\neq\dots\neq s_l }}^{\tau_N(t)}
        \prod_{j=1}^l [ c_N(s_j) - B_n^\prime D_N(s_j) ] \1{E \cap E^\prime} \notag\\
    &\qquad- \sum_{l=1}^{\tau_N(t) -k}
        (-\alpha_n)^l (1+O(N^{-1})) \frac{1}{l!}
        \sum_{\substack{s_1\neq\dots\neq s_l \\ \exists i,i^\prime 
        : s_i=r_{i^\prime} }}
        \prod_{j=1}^l [ c_N(s_j) - B_n^\prime D_N(s_j) ]
        \1{E \cap E^\prime} . \label{eq:031}
\end{align}
...
\leq \Bigg\{ 1+ \sum_{l=1}^{\tau_N(t) -k} (-\alpha_n)^l (1+O(N^{-1})) 
        \frac{1}{l!} t^l \1{E \cap E^\prime} \Bigg\}
        \sum_{\substack{r_1 <\dots< r_k :\\ r_i \leq \tau_N(t_i) \forall i}}
        \prod_{i=1}^k p_{r_i} \notag\\
...
\leq \alpha_n^k \lim_{N\to\infty} \E\left[ \left( 1+
        \sum_{l=1}^{\tau_N(t) -k} (-\alpha_n)^l \frac{1}{l!} t^l \1{E \cap E^\prime} \right)
        \sum_{\substack{r_1 <\dots< r_k :\\ r_i \leq \tau_N(t_i) \forall i}}
...
\leq \alpha_n^k \lim_{N\to\infty} \E \left[
    ...
    &\qquad + \alpha_n^k
        ...
        \lim_{N\to\infty} \E \left[ \I{\tau_N(t)\geq k+l} \1{E \cap E^\prime}
        \sum_{\substack{r_1 <\dots< r_k :\\ r_i \leq \tau_N(t_i) \forall i}}
        \prod_{i=1}^k c_N(r_i) \right] \notag\\
    &\qquad+ \exp[ \alpha_n (1 + B_n^\prime) (t+1) ]
...
\end{align}
where passing the limit and expectation inside the infinite sum is justified by dominated convergence and Fubini; see Lemma~\ref{thm:DCT_Fubini}\ref{thm:DCTFubini_b}.
...
By Lemmata \ref{thm:indicators_tau}, \ref{thm:indicators_cN} and \ref{thm:indicators_DN}, $\lim_{N\to\infty} \Prob[ \{ \tau_N(t)\geq k+l \} \cap E \cap E^\prime ] =1$,  so we can apply Lemma~\ref{thm:induction_sumprodcN} to the remaining expectations in \eqref{eq:040}, yielding
\begin{align}
\lim_{N\to\infty} \E \left[ 
        \sum_{\substack{r_1 <\dots< r_k :\\ r_i \leq \tau_N(t_i) \forall i}}
        \left( \prod_{i=1}^k p_{r_i} \right)
...
\end{equation}
where the $O(N^{-1})$ term does not depend on $r$.
When $N$ is large enough for the factor of $(1+O(N^{-1}))$ to be non-negative, a sufficient condition to ensure the bound in \eqref{eq:050a} is non-negative is the event $E^\prime$ that was defined in \eqref{eq:defn_Eprime}.
...
&- \left( \sum_{s=1}^{\tau_N(t)} D_N(s) \right)
    ...
    &\qquad+ (t+1) \exp[ \alpha_n (1+O(N^{-1})) (t+1) (1+B_n^\prime) ] 
        \Bigg\} .
\end{align}
Due to \citet[Equations (3.3)--(3.5)]{brown2021}, all but the first two lines in the above have vanishing expectation, leaving
\begin{align}
\lim_{N\to\infty} \E &\left[ 
        \sum_{\substack{r_1 <\dots< r_k :\\ r_i \leq \tau_N(t_i) \forall i}}
        \left( \prod_{i=1}^k p_{r_i} \right)
...
\end{align}
Passing the limit and expectation inside the infinite sum is justified by dominated convergence and Fubini; see Lemma~\ref{thm:DCT_Fubini}\ref{thm:DCTFubini_b}.
...
%\end{equation}
By a modification of the upper bound in Lemma~\ref{thm:sumprod1}\ref{thm:sumprod1_b},
\begin{align}
\sum_{\substack{ r_{i_{j-1}+1} \neq \dots \neq r_{i_j} \\ = 
...
&\leq \prod_{j=1}^k \frac{(t_j - t_{j-1})^{i_j - i_{j-1}}}{(i_j - i_{j-1})!}
        + \sum_{\mathcal{I} \subset [k]} c_N(\tau_N(t_{j^*})) 
...
For the corresponding lower bound, by a modification of the lower bound in Lemma~\ref{thm:sumprod1}\ref{thm:sumprod1_b},
\begin{align}
\sum_{\substack{ r_{i_{j-1}+1} \neq \dots \neq r_{i_j} 
        \\ = \tau_N(t_{j-1})+1 }}^{\tau_N(t_j)}
        \prod_{i=i_{j-1}+1}^{i_j} c_N(r_i)
...
(If $t_j = t_{j-1}$ then $E^\star_j$ has probability one automatically; otherwise the constant on the right is strictly positive and so satisfies the conditions of Lemma~\ref{thm:indicators_c2}.)
...
&\geq \prod_{j=1}^k \left\{ \frac{(t_j - t_{j-1})^{i_j - i_{j-1}}}{(i_j - i_{j-1})!}  
        ...
        ( t_j - t_{j-1} +1 )^{i_j - i_{j-1} -2} \right\} \1{E^\star} \notag\\
...
Let $A,B$ be events. 
%\seb{Sequences of events, really. Dependence on some incremental variable is implicit, also in the limit notation.}
...
\begin{proof}
We can replace the event $\{ \tau_N(t) \geq l \}$ with an event of the form of $E$ in Lemma~\ref{thm:indicators_cN}:
\begin{equation}
\{ \tau_N(t) \geq l \} 
...
\begin{lemma}\label{thm:indicators_c2}
Fix $k\in\mathbb{N}$, a sequence of times $0 = t_0 \leq t_1 \leq \cdots \leq t_k \leq t$, and let $K_1,\dots,K_k$ be strictly positive constants such that for each $j$, $K_j^{-1} = O(1)$ as $N\to\infty$.
Define the event
\begin{equation}
E^\star := \bigcap_{j=1}^k \left\{ \sum_{s=\tau_N(t_{j-1})+1}^{\tau_N(t_j)}
...
The first case in the following lemma is used twice in the proof of Lemma \ref{thm:basis}, at \eqref{eq:022} and \eqref{eq:029}.
The second case is used once in Lemma~\ref{thm:inductionUB} at \eqref{eq:043} and once in Lemma~\ref{thm:inductionLB} at \eqref{eq:056}.
\seb{This lemma does not yet appear on the dependencies graph. It precedes lemma 4,5,6 only. I also haven't yet referenced from the mentioned lemmas/equations to here.}
\begin{lemma}\label{thm:DCT_Fubini}
For any fixed $t>0$,
\begin{enumerate}[label=(\alph*)]
\item \label{thm:DCTFubini_a} \hspace{1cm}
$\begin{aligned}
\E \left[ \sum_{l=0}^\infty \left| (-\alpha_n)^l (1+O(N^{-1})) \frac{1}{l!} t^l 
        \right| \right]
< \infty
\end{aligned}$
\item \label{thm:DCTFubini_b} \hspace{1cm}
$\begin{aligned}
...
\begin{proof}
For \textbf{\ref{thm:DCTFubini_a}},
\begin{align}
\E \left[ \sum_{l=0}^\infty \left| (-\alpha_n)^l (1+O(N^{-1})) \frac{1}{l!} t^l 
        \right| \right]
&\leq \E \left[ \sum_{l=0}^\infty \alpha_n^l (1+O(N^{-1})) \frac{1}{l!} t^l 
        \right]
= \E[ \exp\{ \alpha_n t (1+O(N^{-1})) \} ] \notag\\
&\hspace{4cm}= \exp\{ \alpha_n t (1+O(N^{-1})) \}
< \infty .
\end{align}
For \textbf{\ref{thm:DCTFubini_b}},
\begin{align}
\E \left[ \sum_{l=0}^\infty \Bigg| (-\alpha_n)^l (1+O(N^{-1})) \frac{1}{l!} t^l 
        \sum_{\substack{r_1 < \cdots < r_k : \\ r_i \leq \tau_N(t_i) \forall i}}
        \prod_{i=1}^k c_N(r_i) \Bigg| \right]
...
\item Lemma~\ref{thm:DCT_Fubini}\ref{thm:DCTFubini_b} is used in Lemmata \ref{thm:inductionLB} and \ref{thm:inductionUB}.
...
\draw[->] (12,0.2)--(12,-1.6);
\end{tikzpicture}
\caption{Graph showing dependencies between the lemmata used to prove weak convergence. Dotted arrows indicate dependence via a slight modification of the preceding lemma.}
\end{figure}
